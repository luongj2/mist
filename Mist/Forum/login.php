...
try{ 
    ...
    $pdo->begin_transaction();
        $sql =("SELECT * FROM Admin WHERE admin_account = '{$_POST['accountAD']}'");
        $vaild = $pdo->query($sql);
        $pdo->commit();
    ...
    $passwd1 = filter_input(INPUT_POST,'passwdLI');
     ...
     if(!$validmail){
        ...
        $pdo->begin_transaction();
        $sql =("SELECT * , CONCAT(UPPER(userFName),' ',UPPER(userLName)) AS userName1 FROM users WHERE user_account = '$validmail'");
        $vaild = $pdo->query($sql);
        $pdo->commit();
        $row = $vaild->fetch_assoc();
